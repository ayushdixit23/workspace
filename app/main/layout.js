...
export default function MainLayout({ children }) {
  ...
  return (
    <div>
      <div className="bg-[#f1f1f1] vs:max-sm:h-[95.5vh] font-nunito duration-75 min-h-screen w-screen flex overflow-auto no-scrollbar">
        ...
        <div className="w-full overflow-y-scroll no-scrollbar 
          ...
          <div className="bg-[#fafafa]  w-full h-fit z-30 rounded-t-[34px]">
            ...
            <div className={`${path === "/main/community" ? "pn:max-sm:bg-white" : null} flex flex-col bg-[#F8F9FC] rounded-t-[26px] gap-4`}>
              ...
              <div className={`z-20 rounded-xl max-w-full w-full max-h-[83vh] ${path == "/main/dashboard" ? "pn:max-sm:overflow-y-scroll pn:max-sm:no-scrollbar" : "overflow-y-scroll no-scrollbar"} `}>
                {children}
              </div>
            </div>
          </div>
