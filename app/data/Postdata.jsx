  ...
  return (
    <>
      ...
      <Link
        ...
        className="bg-white dark:text-white dark:bg-[#273142] my-3 rounded-xl overflow-hidden sm:hidden"
      >
        <div className="pt-3 px-0 font-bold dark:text-white text-[#030229]">
          Recent Posts
        </div>
        ...
        analyticsdata?.postmerged.filter(
          ...
          <div className="overflow-y-scroll dark:text-white dark:bg-[#273142] rounded-xl bg-white no-scrollbar max-h-[300px] ">
            {analyticsdata?.postmerged
              ?.filter((f) => f?.kind !== "poll")
              ?.filter((w) => w?.community?.title === state.name)
              ?.map((d, i, arr) => (
                <div
                  key={i}
                  className={`${
                    i === arr.length - 1 ? "border-b-0" : null
                  } border-b border-[#eaecf0] px-2 flex flex-col justify-center items-center gap-4 w-full`}
                >
                  ...
                  <div className="flex justify-evenly dark:text-white text-[#101828] mb-3 w-full items-center">
                    ...
                    <div>
                      <div className="bg-[#ecfdf3] p-1 px-2 flex justify-center items-center rounded-xl">
                        ...
                        <div className="text-[#12b76a]">{`${Math.round(
                          parseInt(d?.engrate)
                        )}%`}</div>
                      </div>
      ...
      <Link
        href={`/main/post/${encryptaes(state.id)}`}
        className="h-full pn:max-sm:hidden"
      >
        {!analyticsdata?.postmerged ||
        analyticsdata?.postmerged.length === 0 ||
        analyticsdata?.postmerged
          ?.filter((f) => f?.kind !== "poll")
          .filter((d) => d?.community?.title === state.name).length === 0 ? (
          <NoPost id={encryptaes(state.id)} setOpen={false} />
        ) : (
          <div className="h-[500px] overflow-y-scroll no-scrollbar dark:text-white dark:bg-[#273142] bg-white rounded-xl sm:p-2  w-full">
            <Table className="text-sm hover:bg-none h-full">
                ...
                {analyticsdata?.postmerged
                  ...
                  .map((d, i) => (
                    <TableRow key={i}>
                      ...
                      <TableCell className="text-center">
                        {formatNumber(d?.comments?.length)}
                      </TableCell>
                      <TableCell className="text-center">
                        {formatNumber(d?.sharescount)}
                      </TableCell>
